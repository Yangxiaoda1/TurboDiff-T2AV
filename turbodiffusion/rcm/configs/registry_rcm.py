...
class Config(config.Config):
    ...
    defaults: List[Any] = attrs.field(
        factory=lambda: [
            ...
            {"checkpoint": "local"},
            {"ckpt_type": "dcp"},
            {"model": "fsdp_t2v_distill_rcm"}, # 默认指向 RCM 模型
            {"net": None},
            {"net_teacher": None},
            # 注意：移除了 net_fake_score 和 optimizer_fake_score
...
def make_config() -> Config:
    ...
    register_dataloader()
    register_optimizer()
    # 移除 register_optimizer_fake_score()
    register_scheduler()
    register_callbacks()
    register_checkpoint()
    register_ckpt_type()
    ...
    # 移除 register_net_fake_score()
    register_conditioner()
    register_ema()
    register_tokenizer()

    # experiment config are defined in the experiments folder
    # reload=False 减少 CephFS 并发读，3 机时 config 加载更快，避免部分节点先到 init 等慢节点
    import_all_modules_from_package("rcm.configs.experiments.rcm", reload=False)
    return c
